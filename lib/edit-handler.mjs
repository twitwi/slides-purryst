...
function findAtInLine(lines, startLine) {
  const searchEnd = Math.min(lines.length, startLine + 4)
  for (let j = startLine; j < searchEnd; j++) {
    const m = lines[j]?.match(/at:\s*"([^"]+)"/)
    if (m) return { lineIdx: j, match: m }
  }
  return null
}

function findDragCallAtLine(lines, lineNum) {
  ...
  for (let i = start; i < searchEnd; i++) {
    if (lines[i].includes('#drag(')) {
      const result = findAtInLine(lines, i)
      if (result) return { lineIdx: result.lineIdx, match: result.match }
    }
  }
  return null
}
  ...
  try {
    ...
    if (data.dragId != null && useTypst) {
      ...
      if (data.sourceLine != null) {
        const result = findDragCallAtLine(lines, parseInt(data.sourceLine, 10))
        if (result) {
          lines[result.lineIdx] = lines[result.lineIdx].replace(result.match[0], typstNew)
          found = true
        }
      }
