...
// `rbox: "..."` belonging to the `#drag(` call whose opening paren sits at
// `openParenIdx`, scoped to the call's own paren span (never a following
// drag's). Returns the raw regex match, or null when the call has no `rbox`.
function findAtInCall(content, openParenIdx) {
  ...
  for (let i = openParenIdx; i < content.length; i++) {
    ...
    if (c === '(') depth++
      ...
      depth--
      if (depth === 0) return content.slice(openParenIdx, i).match(/rbox:\s*"([^"]*)"/)
    }
  }
  return null
}
...
// (`#drag[…]`) never carry an `rbox:` argument, so they always come back for
// insertion; paren-form calls get their own `rbox:` (paren-scoped) when present.
...
// Turn a drag call that has no `rbox:` into one that does, preserving its form:
// `#drag[…` → `#drag(rbox: "…")[…`, `#drag(…` → `#drag(rbox: "…"…)`.
...
// Apply a resolved drag edit: replace the call's existing `rbox:` or insert a
...
export function handleEdit(body, rootDir, method, res) {
  ...
  try {
    ...
    if (data.dragId != null && useTypst) {
      const newMatch = data.newAttrs?.match(/rbox="([^"]+)"/) // extract just box info from HTML originating message
      if (!newMatch) { sendJson(res, 400, { error: 'Could not parse rbox value from newAttrs' }); return }
      const newValue = newMatch[1]
      const typstNew = `rbox: "${newValue}"`
    ...
    if (isInsert) {
      const newMatch = newAt?.match(/rbox="([^"]*)"/)
      if (!newMatch) { sendJson(res, 400, { error: 'newAttrs must include rbox="..." attribute' }); return }
      ...
      // An empty `rbox` the author wrote explicitly must be replaced, not
      // duplicated by the append below.
      const existing = /(<sp-drag\s[^>]*?)rbox="[^"]*"/i.exec(slice)
      const replaced = existing
        ? slice.replace(existing[0], `${existing[1]}rbox="${newVal}"`)
        : slice.replace(insertRegex, `<sp-drag${im[1]} rbox="${newVal}"${im[2]}`)
      const after = content.slice(0, blockStart) + replaced + content.slice(blockEnd)
    ...
    if (!oldMatch) { sendJson(res, 400, { error: 'oldAttrs must include rbox="..." attribute' }); return }
    const oldVal = oldMatch[1].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    const newMatch = newAt?.match(atRegex)
    if (!newMatch) { sendJson(res, 400, { error: 'newAttrs must include rbox="..." attribute' }); return }
    const newVal = newMatch[1]
    const tagRegex = new RegExp(`(<sp-drag\\s[^>]*?rbox=")${oldVal}(")`)
    let match = null
    ...
    if (match) {
      ...
      // editableIndex, so overwrite whatever `rbox=` it currently carries
      // (last-write-wins) instead of failing the save.
      const anyAt = /(<sp-drag\s[^>]*?rbox=")[^"]*(")/i.exec(slice)
